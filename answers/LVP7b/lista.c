...
/*sint bolha_lista(lista_t *l, int (*comparar)(const void *a, const void *b)){

	if(!l || lista_vazia(l) || !comparar) return 0;
	...
	while(swap){
		swap =0;
		left = lista_cabeca(l);
		rig = lista_proximo(left);
		
		while(rig != NULL){
		
			if(comparar(lista_dado(left), lista_dado(rig)) == 1){	 	  	 	    	   	     	    		   	       	 	
